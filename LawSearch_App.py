...
import fitz  # PyMuPDF
import docx
from util import extract_text_from_docx, extract_text_from_pdf, extract_text_from_txt, cleaning, generate_embeddings, 
collect_paths, scores, query_weaviate
...
if st.button("Esegui Ricerca"):
    ...
    if not weaviate_filters["operands"]:
        ...
        # Esegui la query
        risultati = query_weaviate(client, model_rag, query, num_results, alpha, {})
    else:
        risultati = query_weaviate(client, model_rag, query, num_results, alpha, weaviate_filters)
    ...
    try:
        ...
        #st.write(list(generate_embeddings(query, model_rag)))
        st.write(risultati)
